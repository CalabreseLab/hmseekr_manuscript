...
filelist05<-dir(path='./k6p0.05/',pattern='*._wide.csv')
...
for (n in 1:length(filelist05)) {
  ...
  temp<-read.csv(paste0('./k6p0.05/',filelist05[n]),header=T)
...
filelist01<-dir(path='./k6p0.01/',pattern='*._wide.csv')

for (n in 1:length(filelist01)) {
  
  temp<-read.csv(paste0('./k6p0.01/',filelist01[n]),header=T)
...
filelist001<-dir(path='./k6p0.001/',pattern='*._wide.csv')

for (n in 1:length(filelist001)) {
  
  temp<-read.csv(paste0('./k6p0.001/',filelist001[n]),header=T)
...
write.csv(fstats,'single_functionaldomain_fisher_allp_hb_k6.csv')
write.csv(combraw,'single_functionaldomain_rawdata_allp_hb_k6.csv',row.names = F)
...
write.csv(comb_wide,'single_functionaldomain_orgdata_allp_hb_k6.csv',row.names = F)
...
ggsave("perct_diff_p0.05_top10_k6.pdf", plot = p, width = 9, height = 8, units = "in")
...
comb_wide<-read.csv('single_functionaldomain_orgdata_allp_hb_k6.csv',header=T)
...
ggsave("GCperct_p0.05_top10_k6.pdf", plot = p, width = 9, height = 8, units = "in")


# plot GC perct and perct_diff as scatter plot

library(ggplot2)

comb_wide<-read.csv('single_functionaldomain_orgdata_allp_hb_k6.csv',header=T)

cor.test(comb_wide$perct_diff_p0.05[which(comb_wide$splice=='unspliced')],comb_wide$GCperct[which(comb_wide$splice=='unspliced')])

cor.test(comb_wide$perct_diff_p0.05[which(comb_wide$splice=='spliced')],comb_wide$GCperct[which(comb_wide$splice=='spliced')])

p<-ggplot(comb_wide,aes(x = perct_diff_p0.05, y = GCperct)) +
  geom_point(color='red',alpha=0.6,size=2) +
  facet_wrap(.~ splice, scales = 'free_x') +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  scale_y_continuous(limits = c(20, 80), breaks = seq(20, 80, 20)) +
  labs(y = "GC percentage", x = "p 0.05 pert diff (act - rep)") +
  theme(
    panel.background=element_rect(fill='white'),
    axis.line.x = element_line(color="black", linewidth = 0.5),
    axis.line.y = element_line(color="black", linewidth = 0.5),
...
ggsave("PerctDiff_GCperct_p0.05_scatter_k6.pdf", plot = p, width = 8, height = 4, units = "in")


cor.test(comb_wide$perct_diff_p0.01[which(comb_wide$splice=='unspliced')],comb_wide$GCperct[which(comb_wide$splice=='unspliced')])

cor.test(comb_wide$perct_diff_p0.01[which(comb_wide$splice=='spliced')],comb_wide$GCperct[which(comb_wide$splice=='spliced')])

p<-ggplot(comb_wide,aes(x = perct_diff_p0.01, y = GCperct)) +
  geom_point(color='red',alpha=0.6,size=2) +
  facet_wrap(.~ splice, scales = "free_x") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  ...
  scale_y_continuous(limits = c(20, 80), breaks = seq(20, 80, 20)) +
  labs(y = "GC percentage", x = "p 0.01 pert diff (act - rep)") +
  theme(
    panel.background=element_rect(fill='white'),
    axis.line.x = element_line(color="black", linewidth = 0.5),
    axis.line.y = element_line(color="black", linewidth = 0.5),
...
ggsave("PerctDiff_GCperct_p0.01_scatter_k6.pdf", plot = p, width = 8, height = 4, units = "in")



cor.test(comb_wide$perct_diff_p0.001[which(comb_wide$splice=='unspliced')],comb_wide$GCperct[which(comb_wide$splice=='unspliced')])

cor.test(comb_wide$perct_diff_p0.001[which(comb_wide$splice=='spliced')],comb_wide$GCperct[which(comb_wide$splice=='spliced')])

p<-ggplot(comb_wide,aes(x = perct_diff_p0.001, y = GCperct)) +
  geom_point(color='red',alpha=0.6,size=2) +
  facet_wrap(.~ splice, scales = "free_x") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  scale_x_continuous(breaks = scales::breaks_pretty(n = 5),
                     labels = scales::label_number(accuracy = 1)) +
  scale_y_continuous(limits = c(20, 80), breaks = seq(20, 80, 20)) +
  labs(y = "GC percentage", x = "p 0.001 pert diff (act - rep)") +
  theme(
    panel.background=element_rect(fill='white'),
    axis.line.x = element_line(color="black", linewidth = 0.5),
    axis.line.y = element_line(color="black", linewidth = 0.5),
...
ggsave("PerctDiff_GCperct_p0.001_scatter_k6.pdf", plot = p, width = 8, height = 4, units = "in")
...
for (n in 1:length(dnames)) {
  ...
  temp<-read.csv(paste0('./k6p0.05/',dnames[n],'_wide.csv'),header=T)
...
for (n in 1:length(dnames)) {
  ...
  temp<-read.csv(paste0('./k6p0.01/',dnames[n],'_wide.csv'),header=T)
...
for (n in 1:length(dnames)) {
  ...
  temp<-read.csv(paste0('./k6p0.001/',dnames[n],'_wide.csv'),header=T)
...
write.csv(comb,'total_unique_coverage_table_k6.csv',row.names = F)
