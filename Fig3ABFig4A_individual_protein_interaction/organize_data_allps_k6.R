...
for (p in plist) {
  ...
  dfname<-paste0('XNM_hmseekrhits_paired_unspliced_pval_CKnormed_',p,'_k6.csv')
  print(dfname)
  ...
  htotalcount<-read.csv(paste0('XNM_hmseekrhits_paired_unspliced_ncount_CKnormed_',p,'_k6.csv'),header=T)
  
  df$hit_totalcount<-htotalcount$AARS
  
  ttotalcount<-read.csv(paste0('XNM_hmseekrhits_paired_unspliced_transcript_count_CKnormed_',p,'_k6.csv'),header=T)
  
  df$transcript_totalcount<-ttotalcount$AARS
  
  medlen<-read.csv(paste0('XNM_hmseekrhits_paired_unspliced_medlen_CKnormed_',p,'_k6.csv'),header=T)
