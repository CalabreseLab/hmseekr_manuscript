...
files <- list.files(
  path = "./k5hits/",
  pattern = "^tsc_condE_newpool_unspliced_.*_5_viterbi_seekr\\.txt$",
  full.names = F
)
...
for (n in 1:length(files)) {
  ...
  hits<-fread(paste0('./k5hits/',files[n]),sep='\t')
  
  chunkname<-files[n]
  chunkname<-gsub('tsc_condE_newpool_unspliced_','',chunkname)
  chunkname<-gsub('_5_viterbi_seekr.txt','',chunkname)
  ...
  if (nrow(hits)<1) {
    ...
    outputname<-gsub('.txt','_hits.bed',files[n],fixed=T)
    fwrite(hitsbed, paste0('./k5bedfiles/',outputname), sep = '\t', quote = FALSE, col.names = FALSE, row.names = FALSE)
...
files <- list.files(
  path = "./k6hits/",
  pattern = "^tsc_condE_newpool_unspliced_.*_6_viterbi_seekr\\.txt$",
  full.names = F
)
...
for (n in 1:length(files)) {
  ...
  hits<-fread(paste0('./k6hits/',files[n]),sep='\t')
  
  chunkname<-files[n]
  chunkname<-gsub('tsc_condE_newpool_unspliced_','',chunkname)
  chunkname<-gsub('_6_viterbi_seekr.txt','',chunkname)
  ...
  if (nrow(hits)<1) {
    ...
    outputname<-gsub('.txt','_hits.bed',files[n],fixed=T)
    fwrite(hitsbed, paste0('./k6bedfiles/',outputname), sep = '\t', quote = FALSE, col.names = FALSE, row.names = FALSE)
