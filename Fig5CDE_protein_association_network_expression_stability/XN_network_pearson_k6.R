...
xn<-xn[,c(1,2,4,3,5)]
write.csv(xn,'XN_vs_all_pearson_r_network_NA0_k6.csv',row.names = F)
...
networkr<-read.csv('XN_vs_all_pearson_r_network_NA0_k6.csv',header=T)
...
p<-ggplot() +
  geom_density(data = xplot, aes(rval,color=grp),linewidth=2) +
  labs(x = "Network Pearson's r values",y = "Density") +
  scale_color_manual(values=c('#1b9e77','#d95f02','#7570b3'))+
  coord_cartesian(xlim=c(-0.25,0.55))+
  scale_x_continuous(breaks=seq(from=-0.25, to=0.55, by=0.25),labels = scales::number_format(accuracy = 0.01))+
...
ggsave("Xlikevsall_pearson_network_density_k6.pdf", plot = p, width = 5.5, height = 5, units = "in")



p<-ggplot() +
  geom_density(data = nplot, aes(rval,color=grp),linewidth=2) +
  labs(x = "Network Pearson's r values",y = "Density") +
  scale_color_manual(values=c('#1b9e77','#d95f02','#7570b3'))+
...
ggsave("Nlikevsall_pearson_network_density_k6.pdf", plot = p, width = 5.5, height = 5, units = "in")
