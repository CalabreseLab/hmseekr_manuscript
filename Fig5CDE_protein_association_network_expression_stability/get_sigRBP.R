...
write.csv(siglist,'top5sigRBP_XNmodule_k6.csv',row.names = F)
...
xsiglist<-xcomb[xcombpval<0.05]
xsiglist<-unique(xsiglist)
write.csv(xsiglist,'top5sigRBP_Xmodule_k6.csv',row.names = F)
...
nsiglist<-ncomb[ncombpval<0.05]
nsiglist<-unique(nsiglist)
write.csv(nsiglist,'top5sigRBP_Nmodule_k6.csv',row.names = F)
