...
unspliced<-read.csv('XN_vs_all_pearson_r_network_NA0_k6.csv',header=T)

nlike<-read.csv('../pattern/k6NEAT1_pattern_matched_p01.csv',header=T)
xlike<-read.csv('../pattern/k6XIST_pattern_matched_p05.csv',header=T)
...
ngene<-data.frame(transcript_name=nlike$seqName[grepl('unspliced',nlike$seqName)]) 
xgene<-data.frame(transcript_name=xlike$seqName[grepl('unspliced|XIST',xlike$seqName)]) 
...
p<-ggplot() +
  ...
  geom_vline(data = xnlines, aes(xintercept = stability), color = "black", linetype = "solid",linewidth=0.8)+
...
ggsave("XNlikevsunspliced_Bruseqstab_k6.pdf", plot = p, width = 5.5, height = 5, units = "in")
...
p<-ggplot() +
  ...
  theme(panel.background=element_rect(fill='white'),
        ...
        axis.text.x=element_text(size=26, angle = 45, vjust=0.5),
        axis.text.y=element_text(size=26))+
  geom_vline(data = xnlines, aes(xintercept = expression), color = "black", linetype = "solid",linewidth=0.8)+
...
ngene<-data.frame(transcript_name=nlike$seqName) 
xgene<-data.frame(transcript_name=xlike$seqName) 
...
ggsave("XNlikevsALL_Bruseqstab_k6.pdf", plot = p, width = 5.5, height = 5, units = "in")
...
ggsave("XNlikevsALL_expression_k6.pdf", plot = p, width = 5.5, height = 5, units = "in")
...
comb<-read.csv('XN_vs_all_pearson_r_network_NA0_k6.csv',header=T)

# add spliced XN like back in
splicedx<-read.csv('../pattern/k6XIST_pattern_matched_p05.csv',header=T)
splicedxname<-splicedx$seqName[!grepl('unspliced',splicedx$seqName)]

splicedn<-read.csv('../pattern/k6NEAT1_pattern_matched_p05.csv',header=T)
splicednname<-splicedn$seqName[!grepl('unspliced',splicedn$seqName)]

splicedname<-unique(c(splicedxname,splicednname))

spliced<-data.frame(transcript=splicedname,
                    XIST_r=rep(NA,times=length(splicedname)),
                    XIST_perct=rep(NA,times=length(splicedname)),
                    NEAT1_r=rep(NA,times=length(splicedname)),
                    NEAT1_perct=rep(NA,times=length(splicedname)))
...
xsimi<-read.csv('../pattern/k6XIST_pattern_matched_p05.csv',header=T)
comb$Xsimi_p0.05[comb$transcript %in% xsimi$seqName]<-'TRUE'

xsimi<-read.csv('../pattern/k6XIST_pattern_matched_p01.csv',header=T)
comb$Xsimi_p0.01[comb$transcript %in% xsimi$seqName]<-'TRUE'

xsimi<-read.csv('../pattern/k6XIST_pattern_matched_p001.csv',header=T)
comb$Xsimi_p0.001[comb$transcript %in% xsimi$seqName]<-'TRUE'

nsimi<-read.csv('../pattern/k6NEAT1_pattern_matched_p05.csv',header=T)
comb$Nsimi_p0.05[comb$transcript %in% nsimi$seqName]<-'TRUE'

nsimi<-read.csv('../pattern/k6NEAT1_pattern_matched_p01.csv',header=T)
comb$Nsimi_p0.01[comb$transcript %in% nsimi$seqName]<-'TRUE'

nsimi<-read.csv('../pattern/k6NEAT1_pattern_matched_p001.csv',header=T)
...
write.csv(comb,'XN_rnetwork_stab_exp_pattern_semiext_k6.csv',row.names = F)
