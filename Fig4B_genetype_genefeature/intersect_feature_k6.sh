...
for feature in "$FEATURE_FOLDER"/*_filtered.bed; do
    ...
    mkdir -p ./k6realdata05/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realdata05/unsplice_${base_feature}_stats.csv"
    ...
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.05.bed; do
        ...
        output="./k6realdata05/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
...
for feature in "$FEATURE_FOLDER"/*_filtered.bed; do
    ...
    # Create a directory in the current folder named after the feature
    mkdir -p ./k6realdata01/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realdata01/unsplice_${base_feature}_stats.csv"
    echo "file,overlap,nonoverlap" > "$csv_file"
    ...
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.01.bed; do
        hits_base=$(basename "$hits" .bed)
        # Define an output file name; adjust the naming as needed
        output="./k6realdata01/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
...
for feature in "$FEATURE_FOLDER"/*_filtered.bed; do
    ...
    # Create a directory in the current folder named after the feature
    mkdir -p ./k6realdata001/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realdata001/unsplice_${base_feature}_stats.csv"
    echo "file,overlap,nonoverlap" > "$csv_file"
    ...
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.001.bed; do
        hits_base=$(basename "$hits" .bed)
        # Define an output file name; adjust the naming as needed
        output="./k6realdata001/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
