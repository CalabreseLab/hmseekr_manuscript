...
for feature in "$FEATURE_FOLDER"/*.bed; do
    ...
    mkdir -p ./k6realtype05/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realtype05/unsplice_${base_feature}_stats.csv"
    echo "file,overlap,nonoverlap" > "$csv_file"
    
    # Loop over each hits bed file in the current directory
    # only do unspliced
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.05.bed; do
        hits_base=$(basename "$hits" .bed)
        # Define an output file name; adjust the naming as needed
        output="./k6realtype05/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
...
for feature in "$FEATURE_FOLDER"/*.bed; do
    ...
    # Create a directory in the current folder named after the feature
    mkdir -p ./k6realtype01/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realtype01/unsplice_${base_feature}_stats.csv"
    echo "file,overlap,nonoverlap" > "$csv_file"
    
    # Loop over each hits bed file in the current directory
    # only do unspliced
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.01.bed; do
        hits_base=$(basename "$hits" .bed)
        # Define an output file name; adjust the naming as needed
        output="./k6realtype01/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
...
for feature in "$FEATURE_FOLDER"/*.bed; do
    ...
    # Create a directory in the current folder named after the feature
    mkdir -p ./k6realtype001/rd_${base_feature}

    # Define a CSV file with header for this feature
    csv_file="./k6realtype001/unsplice_${base_feature}_stats.csv"
    echo "file,overlap,nonoverlap" > "$csv_file"
    
    # Loop over each hits bed file in the current directory
    # only do unspliced
    for hits in XNM/k6bedfiles/*_unspliced_*_seekr_hits_clean_filtered0.001.bed; do
        hits_base=$(basename "$hits" .bed)
        # Define an output file name; adjust the naming as needed
        output="./k6realtype001/rd_${base_feature}/${hits_base}_${base_feature}_intersect.bed"
